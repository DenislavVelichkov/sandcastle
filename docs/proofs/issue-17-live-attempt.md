...
**Result: incomplete.** The installed package proved live interruption, recovery, reviews, and a genuine owner response. Integration of the live candidate did not pass. The failed integration left the disposable target at its original commit; this attempt grants no live integration or automatic app-close claim.
...
The fixture callback now writes stable check evidence at fixture commit `2705287`. This later commit advanced fixture `main` without merging the live candidate. It prepares a new attempt but cannot unblock the recorded integration intent. A fresh live run needs its own explicit execution decision and call budget.
