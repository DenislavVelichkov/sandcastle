...
The fixed follow-up study uses `makeFixedBenchmarkPlan(arms)` and `initializeFixedBenchmark(directory, policyId, arms)`. The repo-local `sandcastle benchmark` command accepts repeated `--arm model:effort` arguments and requires an explicit `gpt-6-sol:high` reference. With five arms, the plan creates 40 slots: four historical cases times two repetitions times five arms, reversing arm order in repetition two. The plan and Standard credit rates are frozen into the protocol hash. `freezeFixedBenchmarkSelection()` chooses a qualifying challenger from the 20 development slots before held-out work; `assessFixedBenchmark()` checks the selected challenger against the 20 held-out slots. It retains Sol High unless all technical, project and review gates pass, token coverage is complete, verified credit-equivalent workflow cost saves at least 20%, and both repetitions have the same cheaper direction. See the [operator guide](workflow-user-guide.md#run-the-bounded-benchmark) for the exact launch command and the separate account-usage interpretation.

The original adaptive protocol remains available through its existing API:

`benchmarkFixtures` fixes the four historical base and reference commits and their grading focus. `benchmarkSlots` declares the 64 slots in order: 28 fixed development, 28 fixed held-out, then eight adaptive. Repetition two reverses the seven fixed configurations. Sol High is arm 5 and the fixed reference. The public `benchmarkProtocolHash` binds the fixture definitions, schedule and seven Standard-tier settings. Before the later live pilot, the host fills [the benchmark manifest template](benchmark-manifest.template.json) with the installed release, account, worker CLI and image, fixture prompts, tools, cache, roles, acceptance gates and protected grader. It keeps that completed file in protected host state and passes its SHA-256 as `conditionsHash` to every evaluation. The ledger rejects a changed hash. A catalog page alone does not prove effective worker settings.
...
`probe` is a trusted project-owned independent check of the first candidate. Return `passed` to finish implementation after one call, `implementation-failure` with a reason and evidence to use the second reserved call, or `environment-failure` to stop without escalating. A fixed arm uses the same model on both calls. An adaptive arm uses its frozen fallback only for that actionable failure. The second call resumes the first session when available; a verified clean candidate permits a recorded diagnostic handoff if the session is absent. The project still runs its ordinary final check, required roles and acceptance through the controller. The host must keep the protected grader, reference and controller outside candidate edit scope. `exportBenchmarkFixture()` exports a historical base as a one-commit repository. Its protected `grade` callback returns `focusPassed`, `otherGatesPassed`, and nonempty `evidence` for both base and known correction; the export requires base focus failure and otherwise viable gates, then correction and gate success. It records both preflight results in the fixture receipt. Invoke it only inside the later pilot's active-time accounting.

After all 28 fixed development slots, call `freezeBenchmarkPair(directory, policyId)`. The operation requires four accepted development evaluations per qualified configuration, Sol High qualification, complete cost coverage and at least 20% conservative savings in every account window for the starting arm. It ranks upper consumption by shorter then longer windows, picks a distinct qualified fallback by first-iteration success then usage, and uses Sol High then declared arm order for ties. Missing proof records fixed-policy retention. The pair and failure-only rule are frozen before held-out evaluations can run. Call `assessBenchmarkPromotion(directory, policyId)` only after all 64 slots. It admits the frozen pair only if all four held-out adaptive evaluations pass the project gates, none lose a matched Sol High case, every account window saves at least 20% under uncertainty, both repetitions save in the same direction, and attributable token coverage is complete. Otherwise it records fixed-policy retention. `admitBenchmarkPolicy()` turns an admitted ledger into a versioned, evidence-bound policy input for an independently gradable regression or bounded feature; it rejects fixed-policy or other classes. `readBenchmark()` returns the machine-readable ledger for the separate report renderer. Neither a synthetic result nor an admitted assessment changes a project's live policy without its explicit owner action.

In the issue #27 source candidate, `pairEvidenceHash` binds the development selection before held-out exposure, and `promotionEvidenceHash` binds the complete assessment. Changed or unbound inputs block reuse; duplicate or missing scheduled slots cannot qualify. Admission requires frozen host/fixture conditions and returns the complete assessment's `evidenceHash`, its `conditionsHash`, and a `policyHash` that also includes the tested `taskClass`. Reporting activities do not change those assessment inputs. Old admitted flags without bindings are insufficient. The [routing guide](workflow-user-guide.md#review-the-routing-decision) records the actual fixed-policy disposition and separate activation requirements.

The retry path checks source-tree changes, so an empty commit cannot authorize escalation. The workflow accepts only `prompt`, `promptFile`, `promptArgs` and `hooks` from its prompt callback; additional options cannot override the selected agent or add hidden retries. This restriction belongs to the opt-in workflow and preserves the ordinary public `run()` options.

The ledger keeps raw account readings beside conservative lower and upper percentage-point consumption. The caller must predeclare each window's reading resolution and duration, and establish whether delayed readings are bounded; `settled: false`, a reset crossing, missing window, zero reference lower bound, unknown token coverage or incomplete case cannot support promotion. The controller's usage state retains token estimates, verified deltas, incomplete invocations and reset history separately. Wrap host work outside a controller invocation—fixture export, preflight, report generation and cleanup—in `withBenchmarkActivity(directory, policyId, label, maxActiveMs, action)`. It reserves time before the action, records its actual elapsed time and outcome afterward, and uses the shared pilot lock; a crash leaves the reservation charged for recovery. Call it after the measured calibration has established the pilot budget. Human-only waiting is separate. Do not infer credit or dollar savings from token totals.

`sandcastle benchmark-report --directory <absolute-pilot-directory> --policy-id <policy> --manifest <completed-manifest.json> --output <directory>` calls `writeBenchmarkReport()` inside `withBenchmarkActivity()`. The report reads the retained ledger, shared budget and manifest, writes local HTML plus JSON/CSV, and performs no model or grader work. The manifest must match the ledger's frozen conditions hash. It presents the stored pair and promotion outcome without running a second assessment. See the [report guide](workflow-user-guide.md#open-and-interpret-the-benchmark-report) for interpretation and export instructions.
